...
export default function BookWorkshopsWizard({ initialForm = 'Warli', onInquirySubmitted }) {
  ...
  // Dynamic derivation of an illustrative planning estimate based on cohort and kits.
  // The backend Request model only stores a single `budget` total — there is no
  // artisan/logistics split field, so we do not fabricate one here.
  let csrBudget = 60000;
        ...
        <div className="p-6 rounded-2xl bg-secondary-container/80 border border-secondary text-on-secondary-container animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-body-sm text-on-surface-variant mt-1">
                Your request for <strong>{selectedForm} {selectedArchetype}</strong> has been sent. You can track its status in <strong>Section 3: Requests</strong>.
              </p>
            ...
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              <div className="p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/30 shadow-xs space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <span className="text-[10px] font-label-caps text-outline uppercase font-semibold">Estimated Budget (from Step 2 &amp; 3 selections)</span>
                ...
                {/* This is a planning estimate only — the backend Request model
                    stores a single `budget` total, not an artisan/logistics split. */}
                <p className="text-[11px] text-outline">
                  Illustrative estimate for planning purposes only. The submitted request records the total budget above.
                </p>
              </div>
            </div>
          )}
              ...
              <div className="p-6 rounded-2xl bg-surface-container-lowest border border-outline-variant/30 space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-outline-variant/20">
                  <div>
                    ...
                    <h4 className="font-headline-sm text-base font-bold text-on-surface">{user?.name || 'Institution'}</h4>
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-outline-variant/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-surface-container-low p-4 rounded-xl">
                  <div>
                    <span className="text-[10px] font-label-caps text-outline uppercase font-semibold block">Estimated Budget</span>
